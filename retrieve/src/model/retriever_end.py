...
class Retriever(nn.Module):
    ...
    def forward(
        ...
        logits = self.pred(h_triple).squeeze(-1)
        probs = torch.sigmoid(logits)

        # Gumbel-Softmax 논문 방식 명확히 구현
        gumbel_noise_1 = -torch.empty_like(probs).exponential_().log()
        gumbel_noise_2 = -torch.empty_like(probs).exponential_().log()

        log_prob = torch.log(probs + 1e-10)
        log_one_minus_prob = torch.log(1 - probs + 1e-10)

        logits_with_noise = torch.stack(
            [(log_prob + gumbel_noise_1) / tau, (log_one_minus_prob + gumbel_noise_2) / tau],
            dim=-1
        )

        z_soft = F.softmax(logits_with_noise, dim=-1)

        z_hard = torch.zeros_like(z_soft)
        z_hard.scatter_(1, z_soft.argmax(dim=-1, keepdim=True), 1.0)

        # 최종 binary indicator 생성
        z_final = (z_hard - z_soft).detach() + z_soft

        # triple의 선택 여부 mask
        triple_selected_mask = z_final[:, 0]

        # logits와 함께 mask 반환
        return logits, triple_selected_mask