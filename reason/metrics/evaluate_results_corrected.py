...
def eval_recall(prediction, answer, double_check):
    prediction = sorted(deepcopy(prediction), key=len, reverse=True)
    matched = 0.
    for a in answer:
        for pred in list(prediction):
            if match(pred, a) or (double_check and (match(a, pred.split('ans:')[-1].strip()) or match(a, pred))):
                matched += 1
...
def eval_hit(prediction, answer, double_check):
    if len(prediction) == 0:
        return 0
    for a in answer:
        if match(prediction[0], a) or (double_check and match(a, prediction[0].split('ans:')[-1].strip())):
            return 1
    return 0
...
def get_all_retrieved_entities(triplet_list):
    ...
    for triplet in triplet_list:
        all_ent.add(triplet[0]); all_ent.add(triplet[2])
    return list(all_ent)

def eval_hal_score(prediction, answer, double_check, good_sample, no_ans, subgraph_ent, stats):
    answer = deepcopy(answer); score = 0
    stats['total_samples'] += 1
    if good_sample:
        stats['total_g_samples'] += 1
        if no_ans:
            stats['g_no_ans'] += 1; return 0, stats
        for pred in prediction:
            stats['total_ans'] += 1; stats['total_g_ans'] += 1
            no_match = True
            for a in list(answer):
                if match(pred, a) or (double_check and (match(a, pred.split('ans:')[-1].strip()) or match(a, pred))):
                    score += 1; stats['g_c'] += 1; no_match = False; answer.remove(a)
                    not_in_graph = True
                    for ent in subgraph_ent:
                        if pred.lower().split('ans:')[-1].strip() in ent.lower() or ent.lower() in pred.lower():
                            not_in_graph = False; stats['g_c_in_graph'] += 1; break
                    if not_in_graph: stats['g_c_out_graph'] += 1
                    break
            if no_match:
                score += -1; stats['g_w'] += 1
                not_in_graph = True
                for ent in subgraph_ent:
                    if pred.lower().split('ans:')[-1].strip() in ent.lower() or ent.lower() in pred.lower():
                        not_in_graph = False; stats['g_w_in_graph'] += 1; break
                if not_in_graph: stats['g_w_out_graph'] += 1
        return score / len(prediction), stats
    else:
        stats['total_b_samples'] += 1
        if no_ans: stats['b_no_ans'] += 1; return 1, stats
        for pred in prediction:
            stats['total_ans'] += 1; stats['total_b_ans'] += 1
            no_match = True
            for ent in subgraph_ent:
                if pred.lower().split('ans:')[-1].strip() in ent.lower() or ent.lower() in pred.lower():
                    score += -1; stats['b_in_graph'] += 1; no_match = False; break
            if no_match:
                score += -1.5
                no_match_ans = True
                for a in list(answer):
                    if match(pred, a) or (double_check and (match(a, pred.split('ans:')[-1].strip()) or match(a, pred))):
                        stats['b_out_graph_c'] += 1; no_match_ans = False; answer.remove(a); break
                if no_match_ans: stats['b_out_graph_w'] += 1
        return score / len(prediction), stats

def _detect_dataset_and_samples_path(predict_file: str):
    pf = predict_file.lower()
    if "webqsp" in pf:
        return "webqsp", "./scored_triples/webqsp_240912_unidir_test.pth"
    if "cwq" in pf:
        return "cwq", "./scored_triples/cwq_240907_unidir_test.pth"
    m = re.search(r"metaqa[\-_]?(1|2|3)hop", pf)
    if m:
        hops = int(m.group(1))
        cands = [
            f"./scored_triples/metaqa_{hops}hop_240912_unidir_test.pth",
            f"./scored_triples/metaqa_{hops}hop_test.pth",
            os.getenv("METAQA_SCORED_TRIPLES", ""),
        ]
        for cand in cands:
            if cand and os.path.exists(cand):
                return f"metaqa-{hops}hop", cand
        raise FileNotFoundError(
            f"[MetaQA] Could not find scored_triples file for {hops}-hop. "
            f"Checked: {cands}. Set METAQA_SCORED_TRIPLES or place the file accordingly."
        )
...
def eval_results(predict_file, cal_f1=True, split=None, subset=False, bad_samples=False, eval_hops=-1):
    ...
    dataset_name, samples_to_eval_path = _detect_dataset_and_samples_path(predict_file)
    ...
    # HAL은 CWQ/WEBQSP만
    triplets_dict = {}
    compute_hal = dataset_name in {"webqsp", "cwq"}
    if compute_hal:
        ...
        for each in triplets:
            if split == '\n':
                ...
                input_triplets = [(t[0], t[1], t[2]) for t in each['scored_triplets']]
    ...
    stats = {'g_no_ans': 0, 'g_c': 0, 'g_w': 0, 'b_no_ans': 0, 'b_in_graph': 0, 'b_out_graph_c': 0, 'b_out_graph_w': 0,
             ...
             'g_c_out_graph': 0, "g_w_out_graph": 0, 'g_c_in_graph': 0, 'g_w_in_graph': 0} if compute_hal else None
    ...
    with open(predict_file, 'r') as f, open(detailed_eval_file, 'w') as f2:
        for line in tqdm(f):
            ...
            double_check = any([kw in question.lower() for kw in ['when', 'what year', 'which year', 'where', 'sport', "what countr", "language", 'nba finals', 'world series']])
            ...
            if cal_f1:
                ...
                if compute_hal and not subset and not bad_samples:
                    subgraph_ent = get_all_retrieved_entities(triplets_dict.get(id, []))
                ...
                total_pred += num_pred; total_answer += num_answer; total_match += matched_1
    ...
    if len(hit_list) == 0:
        null_out = [0] * 13; null_out.append(None); return null_out
    ...
    avg_hal_score = sum(hal_score_list) / len(hal_score_list) if len(hal_score_list) else 0
    ...
    micro_precision = total_match / total_pred if total_pred else 0
    micro_recall = total_match / total_answer if total_answer else 0
    ...
    print(f"Total number of samples: {total_cnt}, no answer samples: {no_ans_cnt}, ratio: {no_ans_cnt / total_cnt if total_cnt else 0}")

    result_name = 'eval_result_corrected.txt'
    eval_result_path = predict_file.replace('predictions.jsonl', result_name)
    with open(eval_result_path, 'w') as f:
        f.write(result_str)
    return avg_hit, avg_f1, avg_precision, avg_recall, num_exact_match, num_totally_wrong, micro_f1, micro_precision, micro_recall, total_cnt, no_ans_cnt, (no_ans_cnt / total_cnt if total_cnt else 0), avg_hal_score, stats
