    ...
    for a in answer:
        if "ans:" in prediction:
            ...
            for each_pred in all_pred:
                if match(each_pred, a) or (double_check and match(a, each_pred.split('ans:')[-1].strip())):
                    return 1
        else:
            if match(prediction, a):
                return 1
...
def eval_f1(prediction, answer, double_check):
    ...
    for a in answer:
        if match(prediction_str, a):
            ...
            for each_pred in list(all_pred):
...
def _detect_samples_path(predict_file: str):
    pf = predict_file.lower()
    if "webqsp" in pf:
        return "./scored_triples/webqsp_240912_unidir_test.pth"
    if "cwq" in pf:
        return "./scored_triples/cwq_240907_unidir_test.pth"
...
def eval_results(predict_file, cal_f1=True, topk=-1, subset=False, bad_samples=False, eval_hops=-1):
    ...
    acc_list, hit_list, f1_list, precission_list, recall_list = [], [], [], [], []

    samples_to_eval_path = _detect_samples_path(predict_file)
    ...
    with open(predict_file, 'r') as f, open(detailed_eval_file, 'w') as f2:
        for line in f:
            ...
            if cal_f1:
                ...
                acc_list.append(acc); hit_list.append(hit)
                f2.write(json.dumps({
                    'id': id,
